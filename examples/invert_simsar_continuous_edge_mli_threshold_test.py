...
def find_mli_pre_shadow_boundary(
    profile_db: np.ndarray,
    post_shadow_pick: Optional[PeakPick],
    *,
    shadow_threshold_db: float = -25.0,
    search_back_pixels: int = 60,
    min_shadow_run_pixels: int = 2,
) -> Optional[PeakPick]:
    """Find the near-range edge of shadow by walking left from the MLI post peak.

    The established post-shadow pick is the anchor and is never changed here.
    Moving toward decreasing range, we look for the first sustained section at
    or below ``shadow_threshold_db``.  The pre-shadow position is the threshold
    crossing at the left side of that same low-amplitude section.

    If no sustained <= threshold section is encountered, no pre-shadow point is
    returned.  This prevents a pre-shadow measurement being invented where the
    observed profile does not contain a convincing shadow.
    """
    ...
    # Walking left from the post-shadow peak, the first shadow encountered is
    # the rightmost sustained <= threshold run in the search interval.
    shadow_a, shadow_b = max(runs, key=lambda run: run[1])
    shadow_a += stop_idx
    shadow_b += stop_idx

    # We need an above-threshold sample immediately before the low run. If the
    # low run reaches the left edge of the search window, its start is unknown.
    left_idx = shadow_a - 1
    right_idx = shadow_a
    if left_idx < 0:
        return None
    ...
    # Linear interpolation gives the -25 dB crossing instead of arbitrarily
    # choosing one of the two neighbouring pixels.
    y0 = float(y[left_idx])
...
def pick_mli_pre_shadow_boundaries(
    profiles_db: Mapping[int, np.ndarray],
    ...
    *,
    ...
    """Pick MLI pre-shadow threshold crossings anchored to post-shadow picks."""
    return {
        row: find_mli_pre_shadow_boundary(
            profiles_db[row],
...
def run_peak_inversion(
    ...
    *,
    ...
    filtered_mli_pre_picks_dense = pick_mli_pre_shadow_boundaries(
        filtered_mli_dense,
        filtered_mli_picks_dense,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "threshold required to call it a shadow. Default: 2."
        ),
    )
